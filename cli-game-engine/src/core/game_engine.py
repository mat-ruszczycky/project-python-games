...
class GameEngine:
    ...
    def __init__(self, state: GameState, inputHandler: InputHandlerInterface, outputHandler: OutputHandlerInterface) -> None:
    ...
    def play(self, gameLoop: callable) -> None:
        ...
        while self.state.isPlaying:
            ...
            while userInput not in ['y', 'n']:
                self.outputHandler.displayMessage("Invalid input. Please enter 'y' to continue or 'n' to stop.")
                userInput = self.inputHandler.getUserInput('Continue playing? (y/n): ')
